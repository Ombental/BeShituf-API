...
We are currently using postgres. You need to set up a user,
   ...
   * create a database, a user and a role
    ```
    CREATE DATABASE BeShituf_db;
    CREATE USER BeShituf_user WITH PASSWORD 'BeShituf_pass';
    ALTER ROLE BeShituf_user SET client_encoding TO 'utf8';
    GRANT ALL PRIVILEGES ON DATABASE BeShituf_db TO BeShituf_user;
    ALTER ROLE BeShituf_user CREATEDB;
   ```
